...
export async function createProduct(unsafeData:Zod.infer<typeof productDetailsSchema>):Promise<{error:boolean,message:string} | undefined>{
...
export async function updateProduct(
    id:string,
    unsafeData:Zod.infer<typeof productDetailsSchema>):Promise<{error:boolean,message:string} | undefined>{
    const {userId}=await auth()
    const {success,data}=productDetailsSchema.safeParse(unsafeData)
...
return {message:isSuccess?"Successfully deleted your product":errorMessage}
}

export async function updateCountryDiscounts(
    id: string,
    unsafeData: Zod.infer<typeof productCountryDiscountsSchema>
  ) {
    const { userId } = await auth()
...
export async function updateProductCustomization(
    id:string,
    unsafeData:Zod.infer<typeof productCustomizationSchema>){
const {userId} =await auth()
